...
class ApiTokenSecretConfigurationTest {
	...
	@Test
	void deve_falhar_no_startup_quando_password_reset_secret_nao_estiver_configurado() {
		var environment = new StandardServletEnvironment();
		environment.getPropertySources().remove(StandardEnvironment.SYSTEM_ENVIRONMENT_PROPERTY_SOURCE_NAME);
		...
		assertThatThrownBy(() -> new SpringApplicationBuilder(DespesasApplication.class)
			...
			.properties(
				...
				"server.port=0",
				"spring.flyway.enabled=false",
				"spring.datasource.url=jdbc:h2:mem:passwordResetSecretStartup;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DEFAULT_NULL_ORDERING=HIGH",
				"spring.datasource.username=sa",
				"spring.datasource.password=",
				"spring.datasource.driver-class-name=org.h2.Driver",
				"spring.jpa.database-platform=org.hibernate.dialect.H2Dialect",
				"spring.jpa.hibernate.ddl-auto=create-drop",
				"app.security.token-secret=test-token-secret"
			)
			.run())
			.hasStackTraceContaining("app.security.password-reset-secret must be configured");
	}
}
